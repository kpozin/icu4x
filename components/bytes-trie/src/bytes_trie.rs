...
// impl<'a> IntoIterator for BytesTrie<'a> {
...
pub type BytesTrieEntry<'a> = (&'a [u8], Option<i32>);

#[derive(Debug)]
pub struct BytesTrieIterator {
    bytes: Vec<u8>,
    offset: Option<usize>,
...
impl BytesTrieIterator {
    fn branch_next(
        ...
        let key_byte = self.bytes[offset];
        offset += 1;
        let node = self.bytes[offset];
    ...
    fn truncate_and_stop(&mut self) -> Option<BytesTrieEntry> {
        self.offset = None;
        (self.key_buffer.as_slice(), None)
    }

    /// Returns an entry with the current key and the given value.
    fn entry_with_value(&self, value: i32) -> Option<BytesTrieEntry> {
        Some((self.key_buffer.as_slice(), Some(value)))
    }
...
impl Iterator for BytesTrieIterator {
    type Item = BytesTrieEntry<'a>;
    ...
    fn next(&mut self) -> Option<Self::Item> {
                        ...
                        if top.remaining_branch_length > 1 {
                            match self.branch_next(top.offset, top.remaining_branch_length) {
                                BranchNextResult::FinalValue(final_value) => {
                                    return self.entry_with_value(final_value);
                                }
                                BranchNextResult::NewOffset(new_offset) => new_offset,
                            }
                        } else {
                            self.key_bytes.push(self.bytes[top.offset]);
                            top.offset + 1
                        }
                    }
                }
        ...
        loop {
            ...
            if node >= CompactValue::MIN_LEAD as u8 {
                ...
                return self.entry_with_value(value);
            } else {
                offset += 1;
            }

            if self.is_at_max_key_length() {
                return self.truncate_and_stop();
            }

            if node < MIN_LINEAR_MATCH as u8 {
                if node == 0 {
                    node = self.bytes[offset];
                    offset += 1;
                }
